...
export default function AppBottomNav({ active = 'tests', onTestsPress, onPracticePress, onStatsPress, onProfilePress }) {
  const { t } = useLanguage();

  const itemClass = (tab) => `flex-1 rounded-xl py-3 items-center ${active === tab ? 'bg-mf-primary' : 'bg-transparent'}`;
  const textClass = (tab) => `font-solway-bold text-xs ${active === tab ? 'text-mf-text' : 'text-mf-secondary'}`;
  ...
  return (
    <View className="w-full px-6 pb-6">
      <View className="w-full rounded-2xl border border-mf-secondary/30 bg-mf-bg/85 p-2 flex-row shadow-2xl">
        ...
        <TouchableOpacity className={itemClass('practice')} onPress={onPracticePress}>
          <Text className={textClass('practice')}>{t('nav.practice')}</Text>
        </TouchableOpacity>
